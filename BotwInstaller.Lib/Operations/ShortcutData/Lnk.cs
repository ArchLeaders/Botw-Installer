...
namespace BotwInstaller.Lib.Operations.ShortcutData
{
    public static class Lnk
    {
        ...
        private static async Task Start(this bool create, LnkInfo info)
        {
            ...
            await Write(start, info.name, info.target, info.icon, info.description);
        }

        private static async Task Write(string location, string name, string target, string icon, string desc = "")
        {
            try
            {
                ...
                // Create lnk file.
                await Proc.Start($"{Initialize.temp}\\lnk.resource", $"/F:\"{location}\\{name}.lnk\" /a:c /T:\"{target}\" /I:\"{icon}\" /D:\"{desc}\"");
            }
            catch (Exception e)
            {
