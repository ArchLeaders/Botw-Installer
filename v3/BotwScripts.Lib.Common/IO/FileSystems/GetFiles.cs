namespace BotwScripts.Lib.Common.IO.FileSystems
{
    public static class Files
    {
        ...
        /// <returns></returns>
        public static IEnumerable<string> GetSafe(string path, string searchPattern = "*.*")
        {
        ...
        /// <param name="searchPattern">Search filter</param>
        /// <returns></returns>
        public static List<string> GetSafeNoYield(string path, string searchPattern = "*.*")
        {
            ...
            foreach (string folder in Directory.EnumerateDirectories(path))
                if (!IsIgnorable(folder))
                {
                    try
                    {
                        foreach (var file in GetSafeNoYield(folder, searchPattern))
                            result.Add(file);
                    }
                    catch { }
                }
